...
Current stable product: Decantr 3.10.0 is the published Authority-Aware UI Change Control line. It ships independent authority axes, framework-specific production topology, route and non-route task preparation, bounded route capsules, verification evidence, and explicit CI v3 while keeping v2 reports as the compatibility default. Product release readiness is separate from model-lift research qualification. No precision, recall, adoption-proof, or general model-improvement claim is allowed without its corresponding evidence.
...
CLI, MCP, CI v3, and Studio adapt these verifier-built values; they must not define competing discovery or proof shapes. Existing v2 machine reports remain the 3.x compatibility default. `decantr ci --report-version v3` is the explicit report upgrade, and `decantr ci init --report-version v3` is required to generate a v3 workflow. No environment variable, package version, or upgrade may select v3 silently.
...
- Decantr 3.10 uses a direct-stable product lane with no RC or `next` lane. Deterministic product, package, documentation, tag, npm, and closeout gates authorize publication. Mixed or failed research evidence narrows or blocks only the model-value claim; it does not retroactively unpublish working product behavior.
