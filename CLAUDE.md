...
Current stable product: Decantr 3.11.3 is the Changed-UI Assurance line. Bare `decantr verify` is zero-write and adoption-free: it scopes the current Git change, selects exactly one changed UI app when provable, and returns at most three source-anchored findings. The patch keeps SvelteKit `+page.svelte` files as taskable implementations and colocated page-data modules as supporting authority. The 3.10 independent authority axes, framework-specific production topology, route and non-route task preparation, bounded route capsules, and Project Health contracts remain the foundation. Explicit CI v3 includes the same change-assurance report while v2 stays the compatibility default. Product release readiness is separate from model-lift research qualification. No precision, recall, adoption-proof, or general model-improvement claim is allowed without its corresponding evidence.

Current research program: `docs/programs/2026-07-22-decantr-3-10-ui-change-control-proof.md` tests whether the shipped **Observe -> Prepare -> Verify -> Report** product materially improves model outcomes. Product 3.10 treats routes, layouts, components, stories, overlays, flows, packages, and runtime states as independent UI surfaces; route inventory is evidence, not the product ontology. A predeclared treatment lift must pass separately for each locked frontier model before any material-improvement claim. The frozen 3.9.4 Day-0 result and 3.10 regression replays are diagnostics, not control/treatment results. Paid model runs require explicit budget authorization. The current no-cost runtime matrix is independently reverified at digest `ce55618610740db332d19c6314f78d0141f440bc5ca04ec79698b3359d6c93bc`; evaluator qualification, runnable task materialization, paid trajectories, and independent human review remain incomplete. None of those research gaps block an honestly bounded product release.
...
| `@decantr/verifier` | `packages/verifier/` | Shared Changed-UI Assurance, discovery, independent UI-surface authority/readiness, verification, governed-change contracts, and report schemas |
| `@decantr/vite-plugin` | `packages/vite-plugin/` | Experimental opt-in Vite guard feedback; not a default product surface |
| `@decantr/cli` | `packages/cli/` | Primary zero-setup changed-UI verifier plus scan/task/full-health/CI, advanced attach, and compatibility workflows |
...
`@decantr/verifier` is the sole owner of the shipped additive governed-change and UI-authority contracts:
...
- `ChangeAssuranceReportV1` records complete Git scope, fail-closed app selection, affected surfaces, independent authority, explicit limitations, and concise source/repair-anchored findings.

CLI, MCP, CI v3, and Studio adapt these verifier-built values; they must not define competing discovery or proof shapes. Bare CLI verify, MCP `decantr_verify` action `changes`, and explicit CI v3 must use the same `ChangeAssuranceReportV1`. Existing v2 machine reports remain the 3.x compatibility default. `decantr ci --report-version v3` is the explicit report upgrade, and `decantr ci init --report-version v3` is required to generate a v3 workflow. No environment variable, package version, or upgrade may select v3 silently.

Stable 3.10 adds a verifier-owned `UISurfaceDiscovery` model and `UISurfaceTaskContextV1` resolver. Keep these evidence axes independent:
...
This pipeline remains supported content/scaffolding behavior, but it is not the Decantr 3.11 product thesis or the primary help path.
...
- Bare `decantr verify` means zero-write Changed-UI Assurance. Git scope must include staged, unstaged, deleted, renamed, untracked, commit-range, and unborn-branch cases. Select one changed UI app only when provable; multi-app ambiguity fails closed. Default output is capped at three consequential findings, and only an explicit `--output` may write. `--full` and existing health-only flags retain the previous Project Health workflow.
- Changed-UI Assurance excludes tests, fixtures, stories, mocks, generated files, build output, and sibling apps from production authority. Directly referenced workspace-package source may supply component authority. Primitive-reuse checks are strongest for JSX/TSX; Angular, Vue, and other template parity must remain an explicit limitation until implemented.
- Shared discovery lives in `@decantr/verifier`. Selected app, surface authority, topology completeness, taskability, component inventory, styling authority, and runtime evidence are independent. Authored TanStack source files remain implementation authority; generated route metadata may corroborate public paths for route groups, pathless layouts, and parameters but never becomes the edit target. React Router object routes resolve nested paths and lazy implementation files; Vue Router object routes are formal evidence; Astro content pages remain distinct from response endpoints; SvelteKit `+page.svelte` files are taskable implementations while colocated page-data modules remain non-taskable supporting authority; pathname-only routes remain inferred evidence. Angular uses the selected build entry and TypeScript import graph to prove router authority, resolves workspace secondary-entry component source and external component resources, treats wildcard fallbacks and unresolved lazy boundaries as non-taskable structural signals, excludes tests/fixtures, and keeps every authority axis visible. Do not add a generic Angular route regex or let dependency-only Tailwind evidence outrank selected-app build/runtime styling.
- Next source declaration and deployment reachability are separate. Evaluate root or `src/` middleware/proxy policy and reachable local helpers: statically resolved 4xx-conditioned routes stay discoverable but non-taskable, while unresolved path-dependent policy lowers route authority and blocks task context. Preserve ordered production stylesheet imports through workspace package exports and exclude App Router server handlers from UI component inventory.
- Attached task capsules must include verifier-ranked implementation, authority, ordered style, and advisory evidence reads without counting `.decantr`, `DECANTR.md`, or `decantr.essence.json` churn as source impact. Blocked or unsupported targets expose no edit read set, and compatibility envelopes must remain within the canonical task budget.
- Project Health must distinguish static heuristics from concrete proof. Broad Brownfield inline-style/accessibility aggregates and server-only localhost fallbacks are advisory; client-reachable localhost, accepted obligation drift, browser/axe failures, host tests, and source-corroborated security evidence retain stronger severity.
...
- Graph construction uses shared route and component discovery for route/page implementation provenance and advisory component nodes even when `.decantr/analysis.json` is absent.
- In stable 3.10.0, attached route-backed `decantr task` and MCP task activation retain the current-graph `TaskCapsuleV1` compatibility path. `decantr task <target>` may also prepare read-only discovery context for routes, components, layouts, overlays, stories, packages, or exact files before adoption. It must use live discovery, never saved analysis as authority, and must fail closed on unknown, ambiguous, unresolved, or non-taskable targets. A blocked CLI task emits structured reasons and exits nonzero.
...
- `decantr ci` and generated CI stay on v2 unless `--report-version v3` is explicit. CI v3 carries `AdoptionTruthV1`, `GovernanceDeltaV1`, and `ChangeAssuranceReportV1`; missing, stale, or incompatible proof remains `not_proven` and non-passing unless `--fail-on none` is explicit.
...
- Brownfield and Hybrid adoption do not create or edit formatter ignore files. Greenfield scaffolding may record generated Decantr artifacts in `.prettierignore` when Prettier or Oxfmt is present.
...
| `docs/programs/` | Separate frontier-model lift research program plus preserved historical programs |
...
- `scripts/benchmark-3-10/` owns the frozen corpus, model lock, protocol, neutral runner, and model-lift claim audit. Validate locally without paid calls; do not start the 320-run experiment without explicit budget authorization and sealed qualification material.
- 3.10 evaluator approval is not qualification. Local host probes are explicitly non-materializable. Runnable task evidence requires the external GitHub-hosted container workflow, exact sealed snapshot packs, strict base-fails/expected-passes polarity, a clean committed controller, a signed execution attestation, and offline-verified provenance bound through task manifests, plans, run records, and release audit. Public and private short-lived producer paths exist, but neither grants qualification without the complete retained chain.
- Host `run-one` records are explicitly test-only. Release-eligible model evidence uses the implemented GitHub-hosted split-stage workflow and separately attested evaluator-free agent image. The agent artifact and image contain no evaluator, expected-patch, qualification-controller, oracle, private-task, or hidden-review bytes; only after the agent exits and its signed output delta is verified may the hidden evaluator artifact be downloaded. No hosted split-stage model run exists yet.
...
- Current 3.10 readiness is 40/40 sole-maintainer-approved evaluator specs, 40/40 sole-maintainer-approved environment specs, and an approved 28-repository Day-0 oracle. All 26 dual-image runtime profiles passed GitHub-hosted run `30577939983`, with exact retained-evidence retries `30581534061` and `30581539933` from source commit `cfcb849454d68f2e9b8ff91fdf6b22751eb016cd`; their retained v3 attestations were independently reverified into locked matrix digest `ce55618610740db332d19c6314f78d0141f440bc5ca04ec79698b3359d6c93bc`. Repair commit `98e92472` closes one oracle-assisted Culinary Platform Next.js adoption failure, but it is not a control/treatment result. There is no complete current-head 40-task materializable evaluator receipt set, frozen candidate tarball set, budget approval, provider run, independent blinded review, power pilot, or qualification statistic. Audited provider adapters, hosted run materialization, and the split-stage executor exist but have no hosted paid evidence. Recompute before repeating these numbers; test-green harness mechanics are not product-value evidence.
- Decantr 3.11 uses a direct-stable product lane with no RC or `next` lane. The coordinated package wave is CLI, verifier, and MCP server only. Deterministic product, package, documentation, changed-UI qualification, tag, npm, and closeout gates authorize publication. Mixed or failed research evidence narrows or blocks only the model-value claim; it does not retroactively unpublish working product behavior.
