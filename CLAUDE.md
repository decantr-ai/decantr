...
- Current 3.10 readiness is 40/40 sole-maintainer-approved evaluator specs, 40/40 sole-maintainer-approved environment specs, and an approved 28-repository Day-0 oracle. The prior 26 single-image profile attestations are superseded; the current dual-image matrix is draft with zero current v3 profile attestations. There are zero materializable evaluator receipts, no frozen candidate tarball set, budget approval, provider run, independent blinded review, power pilot, or qualification statistic. Audited provider adapters and the split-stage executor exist but have no hosted paid evidence. Recompute before repeating these numbers; test-green harness mechanics are not product-value evidence.
