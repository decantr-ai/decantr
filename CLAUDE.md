...
- Candidate attached task capsules must include verifier-ranked implementation, authority, ordered style, and advisory evidence reads without counting `.decantr`, `DECANTR.md`, or `decantr.essence.json` churn as source impact. Blocked or unsupported targets expose no edit read set, and compatibility envelopes must remain within the canonical task budget.
