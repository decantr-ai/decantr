...
export function LoginPage() {
  ...
  const [email, setEmail] = useState('jordan@nestable.co');
  const [password, setPassword] = useState('');

  return (
    <AuthForm
      title="Welcome back"
      subtitle="Sign in to find your next stay"
      onSubmit={e => { e.preventDefault(); login(email, password); navigate('/browse'); }}
      submitLabel="Sign in"
      footer={<>New to Nestable? <Link to="/register" style={{ color: 'var(--d-primary)' }}>Create an account</Link></>}
    >
      <Field label="Email">
      ...
      <Field label="Password">
        <div style={{ position: 'relative' }}>
          ...
          <input className="nm-input" type="password" value={password} onChange={e => setPassword(e.target.value)} style={{ paddingLeft: 36 }} required />
        </div>
      </Field>
