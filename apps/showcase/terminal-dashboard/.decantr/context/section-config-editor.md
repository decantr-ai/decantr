...
**Role:** auxiliary | **Shell:** terminal-split | **Archetype:** config-editor
**Description:** Configuration file editor with tree navigation, diff view, and terminal integration. For managing app configuration in a terminal interface.
...
file-tree, syntax-highlight, diff-view, search-replace, undo-redo, auto-save
...
Layout: main-split → status → hotkeys

### diff (/app/config/diff)

Layout: status → diff → hotkeys
