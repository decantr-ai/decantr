...
- **Sections:** 7 (admin-overview [primary], product-management [auxiliary], order-fulfillment [auxiliary], customer-management [auxiliary], admin-analytics [auxiliary], auth-full [gateway], settings-full [auxiliary])
- **Features:** analytics-state, notifications, quick-actions, bulk-actions, search, inline-editing, image-upload, order-tracking, filtering, export, tagging, data-export, date-range-picker, auth, mfa, oauth, email-verification, password-reset, profile-edit, password-change, mfa-management, session-management, theme-toggle, account-deletion, command-palette
- **Guard mode:** strict
