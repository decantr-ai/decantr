...
const titles: Record<AuthMode, string> = {
  login: 'Sign in to Brightgoods',
  register: 'Create store admin account',
...
const subtitles: Record<AuthMode, string> = {
  login: 'Access your store operations console',
  register: 'Manage products, orders, and customers',
...
export function AuthForm({ mode }: AuthFormProps) {
  const [email, setEmail] = useState('morgan@brightgoods.shop');
    ...
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--d-content-gap)' }}>
      ...
      {mode === 'register' && (
        <div>
          ...
          <input className="d-control" type="text" placeholder="Morgan Rivera" value={name} onChange={e => setName(e.target.value)} />
        </div>
      )}
      ...
      {(mode === 'login' || mode === 'register' || mode === 'forgot') && (
        <div>
          ...
          <input className="d-control" type="email" placeholder="admin@store.com" value={email} onChange={e => setEmail(e.target.value)} />
        </div>
      )}
      ...
      <button className="ea-button-accent" type="submit" style={{ width: '100%', justifyContent: 'center', marginTop: '0.25rem' }}>
        {mode === 'login' && 'Sign in'}
