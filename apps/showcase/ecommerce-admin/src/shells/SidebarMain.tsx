...
import {
  LayoutDashboard, Package, ShoppingBag, Users, BarChart3, Settings, Zap,
  ChevronLeft, ChevronRight, Search, LogOut, Store, Sun, Moon, Warehouse,
} from 'lucide-react';
...
const navGroups = [
  ...
  },
  {
    label: 'Catalog',
    items: [
      { to: '/products', icon: Package, label: 'Products', match: (p: string) => p === '/products' || p.startsWith('/products/') },
      { to: '/inventory', icon: Warehouse, label: 'Inventory', match: (p: string) => p === '/inventory' },
    ],
  },
  {
    label: 'Sales',
    items: [
      { to: '/orders', icon: ShoppingBag, label: 'Orders', match: (p: string) => p === '/orders' || p.startsWith('/orders/') },
      { to: '/customers', icon: Users, label: 'Customers', match: (p: string) => p === '/customers' || p.startsWith('/customers/') },
    ],
  },
  {
    label: 'Account',
...
export function SidebarMain() {
  ...
  useEffect(() => {
    ...
    function handleKeydown(e: KeyboardEvent) {
      ...
      if (combo === 'g p') { navigate('/products'); keys.length = 0; }
      if (combo === 'g o') { navigate('/orders'); keys.length = 0; }
      if (combo === 'g c') { navigate('/customers'); keys.length = 0; }
  ...
  return (
    <div style={{ display: 'flex', height: '100vh' }}>
        ...
        <div style={{ borderTop: '1px solid var(--d-border)', padding: '0.5rem', marginTop: 'auto' }}>
          {user && (
            <div style={{
              ...
              <div className="ea-avatar" style={{ width: 28, height: 28, fontSize: '0.65rem' }}>
                {user.avatar}
              </div>
              {!collapsed && (
                <>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {/* Body */}
        <main className="ea-fade-in" style={{ flex: 1, overflowY: 'auto', padding: '1.5rem' }} key={location.pathname}>
          <Outlet />
        </main>
      </div>
    </div>
