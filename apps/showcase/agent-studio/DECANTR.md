...
- **Theme:** carbon-neon (dark mode, rounded shape)
...
- **Sections:** 8 (agent-studio [primary], prompt-library [auxiliary], tool-registry [auxiliary], eval-suite [auxiliary], trace-viewer [auxiliary], marketing-ai-studio [public], auth-full [gateway], settings-full [auxiliary])
- **Features:** agents, tools, prompts, real-time-preview, keyboard-shortcuts, versioning, diff, search, tags, schemas, analytics, evaluation, comparison, regression-detection, traces, observability, replay, marketing, seo, auth, mfa, oauth, email-verification, password-reset, profile-edit, password-change, mfa-management, session-management, theme-toggle, account-deletion, command-palette
- **Guard mode:** strict
