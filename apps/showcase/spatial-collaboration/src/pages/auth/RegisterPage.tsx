  ...
  return (
    <div className="d-surface carbon-card carbon-fade-slide" style={{ padding: '2rem' }}>
      <div className={css('_flex _col _aic _gap1')} style={{ marginBottom: '1.5rem' }}>
        ...
        <h1 className={css('_fontsemi _textxl')}>Create your workspace</h1>
        <p className={css('_textsm')} style={{ color: 'var(--d-text-muted)' }}>Start collaborating in spatial canvas</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className={css('_flex _col _gap4')} role="form">
        <div className={css('_flex _col _gap1')}>
          <label className={css('_textsm _fontmedium')}>Full Name</label>
          <input className="d-control carbon-input" placeholder="Alex Chen" value={name} onChange={e => setName(e.target.value)} />
        </div>
        <div className={css('_flex _col _gap1')}>
          <label className={css('_textsm _fontmedium')}>Email</label>
          <input className="d-control carbon-input" type="email" placeholder="alex@company.com" value={email} onChange={e => setEmail(e.target.value)} />
        </div>
        <div className={css('_flex _col _gap1')}>
