...
- **Theme:** dopamine (dark mode, rounded shape)
...
- **Sections:** 7 (event-discovery [primary], community-feed [auxiliary], ticket-checkout [auxiliary], organizer-dashboard [auxiliary], marketing-events [public], auth-full [gateway], settings-full [auxiliary])
- **Features:** events, search, calendar, map, filters, feed, discussions, reactions, comments, tickets, checkout, payment, event-management, attendees, analytics, revenue, marketing, seo, auth, mfa, oauth, email-verification, password-reset, profile-edit, password-change, mfa-management, session-management, theme-toggle, account-deletion, command-palette
- **Guard mode:** strict
