...
- **Sections:** 6 (storefront-browse [primary], ecommerce-checkout [auxiliary], order-management [auxiliary], marketing-ecommerce [public], auth-full [gateway], settings-full [auxiliary])
- **Features:** search, filtering, cart, wishlist, product-comparison, payments, address-autocomplete, saved-payments, order-tracking, notifications, marketing, seo, auth, mfa, oauth, email-verification, password-reset, profile-edit, password-change, mfa-management, session-management, theme-toggle, account-deletion, command-palette
- **Guard mode:** strict
