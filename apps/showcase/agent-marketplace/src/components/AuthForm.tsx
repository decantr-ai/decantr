...
export function AuthForm({ title, description, fields, submitLabel, footerLinks, showOAuth }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className={css('_flex _col _gap4')}>
      ...
      {showOAuth && (
        <>
          <div className={css('_flex _col _gap2')}>
            <button type="button" className={css('_wfull') + ' d-interactive'} style={{ justifyContent: 'center' }}>
      ...
      <div className={css('_flex _col _gap3')}>
        {fields.map(field => (
          <div key={field.name} className={css('_flex _col _gap1')}>
            <label
              htmlFor={field.name}
              className={css('_textsm _fontmedium') + ' mono-data'}
      ...
      <button
        ...
        disabled={loading}
      >
        {loading && <Loader2 size={16} className="carbon-skeleton" />}
        {submitLabel}
      </button>
