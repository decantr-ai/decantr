...
export function LoginPage() {
  ...
  const { login } = useAuth();
  const [email, setEmail] = useState('operator@agentops.dev');
  ...
  return (
    <div className="d-surface carbon-card carbon-fade-slide" style={{ padding: '2rem' }}>
      <div className={css('_flex _col _aic _gap1')} style={{ marginBottom: '1.5rem' }}>
        ...
        <p className={css('_textsm')} style={{ color: 'var(--d-text-muted)' }}>Sign in to your agent control panel</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className={css('_flex _col _gap4')} role="form">
        <div className={css('_flex _col _gap1')}>
          ...
          <input
            ...
            type="email"
            placeholder="operator@agentops.dev"
            value={email}
      ...
      <p className={css('_textsm _textc')} style={{ color: 'var(--d-text-muted)', marginTop: '1.5rem' }}>
        No account? <Link to="/register" style={{ color: 'var(--d-accent)', textDecoration: 'none' }}>Deploy your first agent</Link>
      </p>
    </div>
  );
}
