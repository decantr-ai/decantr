...
export function TopNavFooterShell() {
  ...
  return (
    <div className={css('_flex _col _minh0') + ' carbon-canvas'} style={{ minHeight: '100vh' }}>
      ...
      >
        <nav className={css('_flex _aic _jcsb _px6 _py3') + ' container'} style={{ maxWidth: 1200, marginInline: 'auto' }}>
          ...
          {/* Desktop nav */}
          <div className={css('_flex _aic _gap6')} style={{ display: 'var(--desktop-flex, flex)' }}>
            <div className={css('_flex _aic _gap4')}>
              {navLinks.map((link) => (
                <Link
                  key={link.to}
                  to={link.to}
                  className={css('_textsm _fontmedium _trans') + (location.pathname === link.to ? ' ' + css('_fgtext') : ' ' + css('_fgmuted'))}
                >
                  {link.label}
                </Link>
              ))}
      ...
      <footer style={{ borderTop: '1px solid var(--d-border)' }}>
        <div className={css('_flex _aic _jcsb _px6 _py6')} style={{ maxWidth: 1200, marginInline: 'auto' }}>
          <div className={css('_flex _aic _gap2 _textsm _fgmuted')}>
            <Cpu size={16} strokeWidth={1.5} />
            <span>Carbon AI Portal</span>
          </div>
          <div className={css('_flex _aic _gap4')}>
            {footerLinks.map((link) => (
              <Link key={link.to} to={link.to} className={css('_textsm _fgmuted _trans')}>
                {link.label}
              </Link>
            ))}
          </div>
