...
- **Sections:** 7 (metrics-overview-dashboard [primary], log-explorer-pro [auxiliary], trace-explorer [auxiliary], alert-manager-pro [auxiliary], marketing-observability [public], auth-full [gateway], settings-full [auxiliary])
- **Features:** metrics, charts, slo-tracking, real-time, logs, search, structured-queries, context, traces, service-map, latency-analysis, alerts, incidents, rules, on-call, marketing, seo, auth, mfa, oauth, email-verification, password-reset, profile-edit, password-change, mfa-management, session-management, theme-toggle, account-deletion, command-palette
- **Guard mode:** strict
