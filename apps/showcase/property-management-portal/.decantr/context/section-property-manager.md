...
**Role:** auxiliary | **Shell:** sidebar-main | **Archetype:** property-manager
**Description:** Property CRUD and management screens including property list, detail view, create/edit forms, and unit management.

## Quick Start

**Shell:** Collapsible sidebar with header bar and scrollable main content area. Used by saas-dashboard, financial-dashboard, workbench, ecommerce (account pages). (nav: 240px, header: 52px)
**Pages:** 5 (list, detail, create, units, unit-detail)
**Key patterns:** property-grid [moderate], property-detail [moderate], form [complex], unit-detail [moderate]
...
crud, file-upload, search, filters
...
Dashboard grid of all properties with filtering, search, view toggles, and portfolio summary.

**Visual brief:** Dashboard page for all properties. Header row with 'Properties' title, a search input, and an 'Add Property' button. A portfolio summary bar shows total properties, total units, average occupancy, and total monthly revenue. Filter controls below offer search by name, filter by status, and view toggle (cards/table/map). The main area displays property cards in a responsive grid, a data table, or a map view with pins. Pagination controls at the bottom handle large portfolios.

**Components:** Button, icon, Input, Select, Tabs

**Composition:**
```
Header = Row(d-control, space-between) > [Title + AddPropertyButton(d-interactive) + ViewToggle(d-interactive)]
Filters = Row(d-control) > [SearchInput(d-control) + StatusFilter(d-control) + TypeFilter(d-control)]
PropertyGrid = Container(d-section, flex-col, gap-6) > [Header + PortfolioSummary + Filters + PropertyItems + Pagination]
PropertyItems = Grid(d-data, responsive: 1/2/3-col) > PropertyCard[]
PortfolioSummary = Row(d-data) > [TotalProperties + TotalUnits + AvgOccupancy(mono-data) + TotalRevenue(mono-data)]
```

**Layout slots:**
- `grid`: Property cards grid
- `header`: Title, add property button, view toggle
- `search`: Property search input
- `filters`: Property type, status, location filters
- `summary`: Portfolio summary metrics
- `pagination`: Page controls
**Responsive:**
- **Mobile (<640px):** Single-column property cards. Summary metrics in a 2x2 grid. Filters collapse into a filter button. Map view takes full screen.
- **Tablet (640-1024px):** Two-column card grid. Summary in a row. Filters visible. Table view has horizontal scroll.
- **Desktop (>1024px):** Three-column card grid or full-width table. All summary, filters, and pagination visible.


### property-detail

Full property view with details, gallery, metrics, units, financials, documents, and maintenance history.

**Visual brief:** Full property view starting with a header showing property name, address, status badge, and action buttons (Edit, Archive). Below the header, a photo gallery as a horizontal scrollable strip or grid of thumbnails. A metrics row shows key stats: total units, occupancy rate, monthly revenue, NOI. The main content area uses horizontal tabs: Overview (property details and description), Units (unit list), Financials (income/expense summaries), Documents (lease PDFs, insurance), and Maintenance (ticket list). The create preset replaces display fields with an input form.

**Components:** Button, icon, Badge, Tabs

**Composition:**
```
Header = Row(d-surface, space-between) > [PropertyName(heading2) + Address(text-muted) + StatusBadge(d-annotation) + ActionButtons(d-interactive)]
Gallery = Row(d-data, scrollable) > Thumbnail(clickable)[]
MetricsRow = Grid(d-data, 4-col) > MetricCard(d-surface)[]
TabContent = Tabs(d-control) > [OverviewTab + UnitsTab + FinancialsTab + DocumentsTab + MaintenanceTab]
PropertyDetail = Container(d-section, flex-col, gap-6) > [Header + Gallery + MetricsRow + TabContent]
```

**Layout slots:**
- `tabs`: Overview, Units, Financials, Documents, Maintenance tabs
- `units`: Unit list (embedded)
- `header`: Property name, address, edit button, back link
- `gallery`: Property photo gallery
- `metrics`: Key metrics row (occupancy, revenue, expenses, NOI)
- `overview`: Property details (type, year, sqft, beds/baths)
- `timeline`: Property activity history
- `documents`: Property documents
- `financials`: Property P&L summary
- `maintenance`: Recent maintenance tickets
**Responsive:**
- **Mobile (<640px):** Single-column layout. Gallery becomes a swipeable carousel. Tabs become a dropdown selector or horizontally scrollable. Content sections stack fully.
- **Tablet (640-1024px):** Standard layout with scrollable tab bar. Gallery shows 3-4 thumbnails. Content area comfortable width.
- **Desktop (>1024px):** Full layout with all tabs visible. Gallery shows full thumbnail row. Content area can show side-by-side panels within tabs.
...
Single unit details including specs, current tenant, lease info, rent history, maintenance, and documents.

**Visual brief:** Detail view for a single rental unit. Header shows unit number/name, status badge (occupied, vacant, maintenance), and action buttons. A specs section displays key-value pairs: bedroom count, bathroom count, square footage, floor, rent amount. A tenant information card shows the current tenant (avatar, name, contact, lease dates) or 'Vacant' with a list-unit button. Tabbed content below shows: Rent History (payment records), Maintenance (ticket list), and Documents (lease, inspection reports). The view preset is the only preset, displaying all information.

**Components:** Button, icon, Badge, Tabs

**Composition:**
```
Specs = Grid(d-data, key-value) > [Bedrooms + Bathrooms + SqFt(mono-data) + FloorPlan?]
Header = Row(d-surface, space-between) > [UnitNumber(heading2) + StatusBadge(d-annotation) + ActionButtons(d-interactive)]
TabContent = Tabs(d-control) > [LeaseTab + PaymentTab + MaintenanceTab + DocumentsTab]
UnitDetail = Container(d-section, flex-col, gap-6) > [Header + Specs + CurrentTenant? + TabContent]
```

**Layout slots:**
- `tabs`: Rent history, maintenance, documents tabs
- `lease`: Current lease summary
- `specs`: Beds, baths, sqft, floor, amenities
- `header`: Unit number, property name, status badge, edit
- `tenant`: Current tenant info (embedded tenant-card)
- `documents`: Unit-specific documents
- `maintenance`: Maintenance history
- `rent-history`: Payment history for this unit
**Responsive:**
- **Mobile (<640px):** Single-column stacked layout. Specs in a 2-column key-value grid. Tenant card full-width. Tabs become a dropdown or accordion.
- **Tablet (640-1024px):** Standard layout. Specs and tenant info side by side. Tabs in a row.
- **Desktop (>1024px):** Full layout with specs, tenant card, and tabs all comfortably displayed.
