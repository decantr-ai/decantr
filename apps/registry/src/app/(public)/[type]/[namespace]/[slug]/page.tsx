        ...
        {showcaseMeta && (
          <section className={`d-surface ${styles.contentSectionSpacing}`} data-elevation="raised" aria-label="Showcase verification summary">
            <div className="flex flex-col gap-3">
              ...
              {showcaseVerification && (
                <p className={styles.supportingCopy}>
                  Shortlist verification recorded a {showcaseVerification.build.passed ? 'passing' : 'failing'} build in {showcaseVerification.build.durationMs} ms and a {showcaseVerification.smoke.passed ? 'passing' : 'failing'} smoke check in {showcaseVerification.smoke.durationMs} ms, covering {showcaseVerification.smoke.routeDocumentsPassed}/{showcaseVerification.smoke.routeDocumentsChecked} audited route documents and {showcaseVerification.smoke.routeHintsMatched}/{showcaseVerification.smoke.routeHintsChecked.length} compiled route hints, with {showcaseVerification.smoke.fullRouteCoverageOk ? 'full route coverage preserved' : 'partial route coverage only'}. The current build ships {formatBytes(showcaseVerification.smoke.totalAssetBytes)} total assets ({formatBytes(showcaseVerification.smoke.jsAssetBytes)} JS, {formatBytes(showcaseVerification.smoke.cssAssetBytes)} CSS) with {showcaseVerification.smoke.langOk ? 'an explicit language hint' : 'no language hint'}, {showcaseVerification.smoke.viewportOk ? 'a viewport tag present' : 'no viewport tag'}, {showcaseVerification.smoke.charsetOk ? 'a charset declaration' : 'no charset declaration'}, and {showcaseVerification.smoke.cspSignalOk ? 'a CSP signal present' : 'no CSP signal'}, alongside {showcaseVerification.smoke.inlineScriptCount} inline script tag(s), {showcaseVerification.smoke.externalScriptsWithoutIntegrityCount} external script(s) without integrity, {showcaseVerification.smoke.externalStylesheetsWithoutIntegrityCount} external stylesheet(s) without integrity, {showcaseVerification.smoke.externalScriptsWithInsecureTransportCount} external script(s) over insecure transport, {showcaseVerification.smoke.externalStylesheetsWithInsecureTransportCount} external stylesheet(s) over insecure transport, {showcaseVerification.drift.inlineStyleCount} inline-style signals, and {showcaseVerification.drift.hardcodedColorCount} hardcoded-color signals.
                </p>
              )}
            </div>
          </section>
