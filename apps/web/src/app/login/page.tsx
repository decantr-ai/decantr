  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: 'var(--bg)' }}>
      <Card className="w-full max-w-md">
        <h1 className="text-2xl font-bold text-center mb-6" style={{ color: 'var(--fg)' }}>
          {mode === 'login' ? 'Sign in to Decantr' : 'Create your account'}
        </h1>
        ...
        <div className="relative mb-6">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="px-2" style={{ background: 'var(--bg-surface)', color: 'var(--fg-dim)' }}>or</span>
          </div>
        </div>
        ...
        <form onSubmit={handleEmailAuth} className="space-y-4">
          ...
          {error && (
            <p className="text-sm" style={{ color: error.includes('Check your email') ? 'var(--success)' : 'var(--error)' }}>
              {error}
            </p>
          )}
        ...
        <p className="text-center mt-4 text-sm" style={{ color: 'var(--fg-muted)' }}>
          {mode === 'login' ? (
            <>Don&apos;t have an account?{' '}
              <button onClick={() => setMode('signup')} className="underline cursor-pointer" style={{ color: 'var(--secondary)' }}>Sign up</button>
            </>
          ) : (
            <>Already have an account?{' '}
              <button onClick={() => setMode('login')} className="underline cursor-pointer" style={{ color: 'var(--secondary)' }}>Sign in</button>
            </>
          )}
        </p>
      </Card>
