...
- **Blueprint:** fractional-ownership
- **Theme:** fintech (dark mode, sharp shape)
- **Personality:** Fractional asset ownership platform with institutional-grade financial visualizations. Cap tables display share classes with vesting chips and fully-diluted percentages. Valuation charts project with confidence bands. Secondary market shows order books with live spreads. Governance ballots let token holders vote on major decisions with weighted voting. Dividend ledgers track distributions with tax treatments. Think Carta meets Rally. Lucide icons. Sharp.
- **Sections:** 7 (asset-browser [primary], ownership-dashboard [auxiliary], secondary-market [auxiliary], governance-center [auxiliary], marketing-fractional [public], auth-full [gateway], settings-full [auxiliary])
- **Features:** browse, valuation, search, filters, ownership, distributions, portfolio, cap-table, trading, order-book, market-data, governance, voting, proposals, marketing, landing, conversion, auth, mfa, oauth, email-verification, password-reset, profile-edit, password-change, mfa-management, session-management, theme-toggle, account-deletion
- **Guard mode:** strict
