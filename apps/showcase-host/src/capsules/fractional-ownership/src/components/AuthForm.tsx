...
const subtitles: Record<AuthMode, string> = {
  login: 'Access your fractional portfolio',
  register: 'Start investing in premium assets',
    ...
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--d-content-gap)' }}>
      ...
      {mode === 'register' && (
        <div>
          ...
          <input className="d-control" type="text" placeholder="Cassandra Voss" value={name} onChange={e => setName(e.target.value)} />
        </div>
      )}
      ...
      {(mode === 'login' || mode === 'register' || mode === 'reset') && (
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '0.25rem' }}>
            ...
            {mode === 'login' && (
              <Link to="/forgot-password" style={{ fontSize: '0.75rem', color: 'var(--d-primary-hover)', textDecoration: 'none' }}>Forgot?</Link>
            )}
          </div>
      ...
      {mode === 'mfa-setup' && (
        <div style={{
          ...
          <div className="fo-mono" style={{ fontSize: '0.7rem', color: 'var(--d-text-muted)' }}>
            FRAC2FA8JKMXP9Q
          </div>
        </div>
      )}

      <button className="fo-button-primary" type="submit" style={{ width: '100%', justifyContent: 'center', marginTop: '0.25rem' }}>
        {mode === 'login' && 'Sign in'}
      ...
      <p style={{ textAlign: 'center', fontSize: '0.8rem', color: 'var(--d-text-muted)' }}>
        {mode === 'login' && (
          <>Don't have an account? <Link to="/register" style={{ color: 'var(--d-primary-hover)', textDecoration: 'none' }}>Create one</Link></>
        )}
        {mode === 'register' && (
          <>Already have an account? <Link to="/login" style={{ color: 'var(--d-primary-hover)', textDecoration: 'none' }}>Sign in</Link></>
        )}
        {(mode === 'forgot' || mode === 'reset') && (
          <Link to="/login" style={{ color: 'var(--d-primary-hover)', textDecoration: 'none' }}>Back to sign in</Link>
        )}
        {(mode === 'verify-email' || mode === 'mfa-setup' || mode === 'mfa-verify' || mode === 'phone-verify') && (
          <Link to="/login" style={{ color: 'var(--d-primary-hover)', textDecoration: 'none' }}>Cancel</Link>
        )}
      </p>
    </form>
  );
