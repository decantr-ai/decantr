...
import {
  TrendingUp, PieChart, List, Target, Shield, Download,
  ArrowRight, Wallet, Check,
} from 'lucide-react';
    ...
    <div>
      ...
      <nav className="fd-nav">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
          ...
          <button className="fd-button-accent" onClick={cta} style={{ padding: '0.375rem 0.875rem', fontSize: '0.875rem' }}>
      ...
      <section className="fd-hero">
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 820, margin: '0 auto' }}>
          <div className="d-annotation" data-status="info" style={{ marginBottom: '1.25rem', display: 'inline-flex' }}>
            New: Tax-ready 1099 exports for 2025
          </div>
          <h1 style={{ fontSize: 'clamp(2.25rem, 5.5vw, 3.75rem)', fontWeight: 700, lineHeight: 1.05, marginBottom: '1.25rem', letterSpacing: '-0.02em' }}>
            Your wealth, <span className="fd-gradient-text">in precise focus</span>
          </h1>
          <p style={{ fontSize: '1.125rem', lineHeight: 1.6, color: 'var(--d-text-muted)', maxWidth: 640, margin: '0 auto 2rem' }}>
            Portfolio tracking, transaction ledgers, budgets, and investment performance — unified into one dashboard built for trust and precision. Every number to two decimal places.
          </p>
          <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center', flexWrap: 'wrap' }}>
            <button className="fd-button-accent" onClick={cta} style={{ padding: '0.625rem 1.5rem', fontSize: '0.9rem' }}>
      ...
      <section id="features" style={{ padding: '5rem 2rem' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
            ...
            <p style={{ color: 'var(--d-text-muted)', maxWidth: 520, margin: '0 auto' }}>
              From daily transactions to quarterly rebalancing — consolidate your financial life in one authoritative view.
            </p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.25rem' }}>
            {marketingFeatures.map(f => {
              ...
              return (
                <div key={f.title} className="fd-card" style={{ padding: 'var(--d-surface-p)' }}>
                  <div style={{
                    width: 40,
                    height: 40,
        ...
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '2rem', textAlign: 'center' }}>
            {marketingStats.map(s => (
              <div key={s.label}>
                <div className="fd-gradient-text fd-mono" style={{ fontSize: '2.25rem', fontWeight: 700 }}>
                  {s.value}
                </div>
      ...
      <section id="pricing" style={{ padding: '5rem 2rem' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
            <h2 style={{ fontSize: '1.875rem', fontWeight: 600, marginBottom: '0.5rem' }}>Straightforward pricing</h2>
            <p style={{ color: 'var(--d-text-muted)' }}>Start free. Upgrade when your portfolio grows.</p>
          </div>
            ...
            {pricingTiers.map(tier => (
              ...
              >
                ...
                <div style={{ marginBottom: '1rem' }}>
                  {tier.price > 0 ? (
                    <>
                      <span className="fd-mono" style={{ fontSize: '1.875rem', fontWeight: 700 }}>
                        ${tier.price.toFixed(2)}
                      </span>
                      <span style={{ fontSize: '0.75rem', color: 'var(--d-text-muted)' }}> {tier.period}</span>
                    </>
                  ) : (
                    <span className="fd-mono" style={{ fontSize: '1.875rem', fontWeight: 700 }}>Free</span>
                  )}
                </div>
                ...
                <button
                  onClick={cta}
                  className={tier.recommended ? 'fd-button-accent' : 'd-interactive'}
                  style={{ width: '100%', justifyContent: 'center', padding: '0.5rem' }}
                >
                  {tier.current ? 'Current plan' : tier.name === 'Advisor' ? 'Contact sales' : 'Get started'}
                </button>
              </div>
            ))}
          </div>
            ...
            {testimonials.map(t => (
              <div key={t.author} className="d-surface" style={{ padding: 'var(--d-surface-p)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
                  <div className="fd-avatar" style={{ width: 32, height: 32, fontSize: '0.7rem' }}>{t.avatar}</div>
                  <div>
      ...
      <section style={{ padding: '6rem 2rem', textAlign: 'center' }}>
        <div style={{ maxWidth: 600, margin: '0 auto' }}>
          <h2 style={{ fontSize: '2rem', fontWeight: 700, marginBottom: '1rem' }}>
            See your whole portfolio today
          </h2>
          <p style={{ fontSize: '1rem', color: 'var(--d-text-muted)', lineHeight: 1.7, marginBottom: '2rem' }}>
            Link your first account in 90 seconds. No credit card required.
          </p>
          <button className="fd-button-accent" onClick={cta} style={{ padding: '0.75rem 1.75rem', fontSize: '0.9rem' }}>
      ...
      <footer style={{ borderTop: '1px solid var(--d-border)', padding: '3rem 2rem 2rem' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '2rem', marginBottom: '2rem' }}>
            {[
              { heading: 'Product', links: ['Portfolio', 'Budgets', 'Investments', 'Changelog'] },
              { heading: 'Company', links: ['About', 'Security', 'Careers', 'Contact'] },
              { heading: 'Resources', links: ['Help Center', 'Guides', 'API', 'Status'] },
              { heading: 'Legal', links: ['Privacy', 'Terms', 'Disclosures', 'SOC 2'] },
            ].map(col => (
              <div key={col.heading}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '1.5rem', borderTop: '1px solid var(--d-border)', flexWrap: 'wrap', gap: '1rem' }}>
            <span style={{ fontSize: '0.75rem', color: 'var(--d-text-muted)' }}>2026 Atlas Wealth Inc. All rights reserved. Not a registered investment advisor.</span>
