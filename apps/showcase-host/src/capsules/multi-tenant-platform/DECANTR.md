...
- **Theme:** launchpad (dark mode, rounded shape)
...
- **Sections:** 8 (tenant-switcher-dashboard [primary], api-console [auxiliary], usage-billing [auxiliary], webhook-manager [auxiliary], audit-center [auxiliary], marketing-platform [public], auth-flow-enterprise [gateway], settings-full [auxiliary])
- **Features:** multi-tenant, org-switcher, team-management, roles, api-console, api-keys, webhooks, documentation, billing, usage-metering, invoices, plans, event-filtering, retry-logic, audit-trail, compliance, export, retention, marketing, seo, auth, sso, mfa, invite-flow, profile-edit, password-change, mfa-management, session-management, theme-toggle, account-deletion, command-palette
- **Guard mode:** strict
