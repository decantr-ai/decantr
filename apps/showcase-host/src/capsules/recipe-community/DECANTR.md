...
- **Theme:** auradecantism (light mode, rounded shape)
...
- **Sections:** 7 (recipe-browser [primary], cook-mode [auxiliary], recipe-creator [auxiliary], recipe-social [auxiliary], marketing-recipe [public], auth-full [gateway], settings-full [auxiliary])
- **Features:** search, filter, gallery, sharing, timer, voice-control, wake-lock, drag-drop, image-upload, autosave, preview, follow, reactions, collections, marketing, seo, auth, mfa, oauth, email-verification, password-reset, profile-edit, password-change, mfa-management, session-management, theme-toggle, account-deletion, command-palette
- **Guard mode:** strict
