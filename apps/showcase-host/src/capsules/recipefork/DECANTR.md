...
- **Theme:** recipefork (light mode, rounded shape)
...
- **Sections:** 8 (recipefork-landing [public], recipefork-recipe-detail [primary], recipefork-social [auxiliary], recipefork-ai-kitchen [auxiliary], recipefork-cookbooks [auxiliary], recipefork-recipe-authoring [auxiliary], recipefork-profile [auxiliary], recipefork-auth [gateway])
...
|-------|--------|----------|---------|
| `.recipefork-surface` | Use on app roots and wide content surfaces where food photography and forms should carry the visual weight rather than decorative theme effects. | Page roots, Shell body regions | background-color: var(--d-bg, #FFFFFF); color: var(--d-fg, #111827); min-height: 100vh |
| `.recipefork-nav` | Use for sticky product nav bars where route links, theme toggle, profile entry, and create actions share a single 64px row. | Top navigation, Sticky headers | backdrop-filter: blur(12px); background: color-mix(in srgb, var(--d-bg) 92%, transparent); border-bottom: 1px solid var(--d-border) |
| `.recipefork-card` | Use for cookbook cards, feed cards, stat blocks, and editor sections that should feel product-grade but not ornamental. | Cards, Form sections, Sidebar blocks | background: var(--d-surface, #FFFFFF); border: 1px solid var(--d-border, #E5E7EB); border-radius: 8px; transition: box-shadow 150ms ease, transform 150ms ease |
| `.recipefork-input` | Use on all recipe authoring and auth controls to keep the product visually consistent across simple and Chef Mode surfaces. | Inputs, Selects, Textareas | border: 1px solid var(--d-border, #E5E7EB); border-radius: 8px; background: var(--d-surface, #FFFFFF); box-shadow: 0 0 0 0 transparent |
| `.recipefork-photo` | Use for hero images, cookbook covers, and generated recipe previews where the photo is the visual anchor. | Recipe hero surfaces, Cookbook cover imagery, Photo-to-recipe previews | border-radius: 12px; overflow: hidden; background: linear-gradient(to top, rgba(0,0,0,0.25), transparent) |
| `.recipefork-editor` | Use for optional story editors and long-form recipe notes without overwhelming the surrounding authoring form. | Story editors, Expandable narrative blocks | border: 1px dashed var(--d-border, #E5E7EB); border-radius: 12px; background: color-mix(in srgb, var(--d-accent) 14%, transparent) |
