...
const titles: Record<AuthMode, string> = {
  login: 'Sign in to Cornerstone',
  register: 'Create your portal account',
...
const subtitles: Record<AuthMode, string> = {
  login: 'Access your portfolio, properties, and residents',
  register: 'Manage properties with confidence',
...
export function AuthForm({ mode }: AuthFormProps) {
  const [email, setEmail] = useState('elena@cornerstone-properties.com');
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--d-content-gap)' }}>
      <div style={{ textAlign: 'center', marginBottom: '0.5rem' }}>
        ...
        <h1 style={{ fontSize: '1.25rem', fontWeight: 600, color: 'var(--d-primary)' }}>{titles[mode]}</h1>
      ...
      {mode === 'register' && (
        <div>
          ...
          <input className="d-control" type="text" placeholder="Elena Whitfield" value={name} onChange={e => setName(e.target.value)} />
        </div>
      )}
      ...
      {mode === 'phone-verify' && (
        <div>
          ...
          <input className="d-control" type="tel" placeholder="+1 555 555 5555" value={phone} onChange={e => setPhone(e.target.value)} />
        </div>
      )}
      ...
      {(mode === 'verify-email' || mode === 'mfa-verify' || mode === 'phone-verify') && (
        <div>
          ...
          <input
            ...
            style={{ fontSize: '1.25rem', letterSpacing: '0.3em', textAlign: 'center' }}
          />
        </div>
      )}
      ...
      {mode === 'mfa-setup' && (
        <div style={{
          padding: '1rem', background: 'var(--d-surface-raised)',
      ...
      <button className="pm-button-primary" type="submit" style={{ width: '100%', justifyContent: 'center', marginTop: '0.25rem', padding: '0.625rem' }}>
        {mode === 'login' && 'Sign in'}
