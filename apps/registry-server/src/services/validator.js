...
export const VALID_TYPES = ['style', 'recipe', 'pattern', 'archetype', 'plugin', 'template', 'vignette'];
...
const BUILT_IN_IDS = {
  ...
  archetype: ['ecommerce', 'saas-dashboard', 'portfolio', 'content-site', 'workbench', 'financial-dashboard', 'recipe-community', 'gaming-platform', 'creative-tool'],
  plugin: [],
  template: ['saas-dashboard', 'ecommerce', 'portfolio', 'content-site', 'landing-page'],
  vignette: ['saas-dashboard', 'ecommerce', 'portfolio', 'content-site', 'workbench', 'financial-dashboard', 'gaming-platform', 'recipe-community', 'ecommerce-admin', 'cloud-platform'],
};
...
function validateVignetteArtifact(content) {
  const errors = [];
  const warnings = [];
  let data;
  try { data = JSON.parse(content); } catch {
    return { valid: false, errors: ['Vignette must be valid JSON'], warnings };
  }
  if (!data.id) errors.push('Vignette must have an "id" field');
  if (!data.name) errors.push('Vignette must have a "name" field');
  if (!data.compose || !Array.isArray(data.compose) || data.compose.length === 0) {
    errors.push('Vignette must have a non-empty "compose" array');
  }
  if (data.dependencies !== undefined && typeof data.dependencies !== 'object') {
    errors.push('Vignette "dependencies" must be an object');
  }
  return { valid: errors.length === 0, errors, warnings };
}
...
export function validateArtifact(type, content) {
  ...
  switch (type) {
    ...
    case 'vignette': return validateVignetteArtifact(content);
