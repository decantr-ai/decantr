...
Private evaluator/oracle material must not be committed to the public repository, mounted into a model container, or placed in a public artifact. The private producer and split-input staging workflow retain private inputs only as three-day artifacts in `decantr-ai/decantr-qualification-private`, verify the source workflow/run/commit, and publish separate agent-safe and evaluator-only artifacts. Do not dispatch the sealed path from the public repository or substitute a public artifact.
