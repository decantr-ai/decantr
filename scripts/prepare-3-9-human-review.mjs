...
const paths = {
  ...
  candidateReplay: resolve(reviewDirectory, 'replays', 'candidate-3.9.1.json'),
};
...
const worksheetPaths = {
  ...
  candidateReplay: 'fixtures/qualification/3.9/review/replays/candidate-3.9.1.json',
};
...
Workflow:
  ...
  3. One registered reviewer fills all 200 adjudication decisions and every case resolution,
     ...
     --seal-adjudication.
  4. Replay public npm 3.8.3 first and candidate 3.9.1 second. The replay process must fill
...
function validateReplay(
  ...
  const context = replayId === 'public383' ? 'public-3.8.3.json' : 'candidate-3.9.1.json';
  if (
    !exactKeys(
      replay,
      [
