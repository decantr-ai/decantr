...
- Create and push the exact stable `v3.9.2` tag only after every gate passes.
- Create the GitHub Release from `docs/releases/2026-07-16-decantr-3-9-0-governed-change-proof.md`.
- Dispatch `.github/workflows/publish.yml` from `main` with `release_tag=v3.9.2`; use the wrapper, never bare `npm publish`.
- `pnpm release:verify`
- `pnpm release:closeout -- --version 3.9.2`
- `pnpm release:announce -- --version 3.9.2 --send` only after closeout passes.
