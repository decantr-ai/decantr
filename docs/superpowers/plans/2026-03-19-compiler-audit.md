# Compiler Audit Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create an automated audit script that tests all 13 Decantr projects against the experimental compiler and generates a checklist report.

**Architecture:** A single Node.js script discovers projects, runs 4 test phases (baseline build, experimental build, syntax check, import test), collects results into JSON, and generates a markdown checklist report.

**Tech Stack:** Node.js (ES modules), child_process for running builds, fs for file operations.
...
| File | Purpose |
|------|---------|
| `tools/compiler-audit.js` | Main audit script with discovery, phases, and report generation |
| `docs/superpowers/specs/2026-03-19-compiler-audit.md` | Generated checklist report |

---

### Task 1: Create Audit Script Skeleton

**Files:**
- Create: `tools/compiler-audit.js`

- [ ] **Step 1: Create the audit script with imports and main structure**
...
Expected: Output shows "Found 0 projects to audit"

- [ ] **Step 3: Commit skeleton**

```bash
git add tools/compiler-audit.js
git commit -m "feat(audit): add compiler audit script skeleton"
```
...
- [ ] **Step 1: Implement discoverProjects function**

Replace the `discoverProjects` function with:
...
- [ ] **Step 2: Verify discovery finds all 13 projects**

Run: `node tools/compiler-audit.js`
Expected: Output shows "Found 13 projects to audit" (may vary if some projects missing src/app.js)

- [ ] **Step 3: Commit discovery**

```bash
git add tools/compiler-audit.js
git commit -m "feat(audit): implement project discovery"
```

---

### Task 3: Implement Build Phase Runner

**Files:**
- Modify: `tools/compiler-audit.js`

- [ ] **Step 1: Add helper to run shell commands**
...
- [ ] **Step 2: Implement Phase 1 (baseline) and Phase 2 (experimental)**
...
- [ ] **Step 3: Commit build phases**

```bash
git add tools/compiler-audit.js
git commit -m "feat(audit): implement build phase runners"
```

---

### Task 4: Implement Syntax and Import Phases

**Files:**
- Modify: `tools/compiler-audit.js`

- [ ] **Step 1: Implement Phase 3 (syntax check)**
...
- [ ] **Step 2: Implement Phase 4 (import test)**
...
- [ ] **Step 3: Commit syntax and import phases**

```bash
git add tools/compiler-audit.js
git commit -m "feat(audit): implement syntax and import phases"
```

---

### Task 5: Implement Project Auditing

**Files:**
- Modify: `tools/compiler-audit.js`

- [ ] **Step 1: Implement auditProject function**

Replace the `auditProject` function:
...
Run: `node tools/compiler-audit.js 2>&1 | head -30`
Expected: Shows audit progress for first few projects

- [ ] **Step 3: Commit audit implementation**

```bash
git add tools/compiler-audit.js
git commit -m "feat(audit): implement full project auditing"
```
...
- [ ] **Step 2: Implement writeReport function**
...
- [ ] **Step 3: Implement printSummary function**
...
- [ ] **Step 4: Commit report generation**

```bash
git add tools/compiler-audit.js
git commit -m "feat(audit): implement report generation"
```

---

### Task 7: Run Full Audit and Commit Report

**Files:**
- Create: `docs/superpowers/specs/2026-03-19-compiler-audit.md` (generated)

- [ ] **Step 1: Run the full audit**

Run: `node tools/compiler-audit.js`
Expected: Audits all projects, generates report

- [ ] **Step 2: Review the generated report**

Run: `cat docs/superpowers/specs/2026-03-19-compiler-audit.md`
Expected: Markdown checklist with all projects, pass/fail status, error details

- [ ] **Step 3: Commit the audit report**

```bash
git add docs/superpowers/specs/2026-03-19-compiler-audit.md
git commit -m "docs: add initial compiler audit results"
```
...
1. Run `node tools/compiler-audit.js` — should complete without script errors
2. Check `docs/superpowers/specs/2026-03-19-compiler-audit.md` exists and has proper formatting
3. Verify checklist shows correct pass/fail status for known projects (portfolio, saas-dashboard should pass; gaming-platform should show pre-existing)
