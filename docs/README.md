...
**Decantr 3.11.3 is the current stable release. Its default entry point is zero-setup Changed-UI Assurance: run `decantr verify` in a Git worktree to inspect only the current UI change with zero writes.**
...
- [Decantr 3.11.3 SvelteKit task-authority patch](releases/2026-08-08-decantr-3-11-3-sveltekit-task-authority.md): current stable delivery patch that separates taskable page implementations from supporting page-data authority.
- [Decantr 3.11.2 route-authority hardening](releases/2026-08-08-decantr-3-11-2-route-authority-hardening.md): TanStack, Astro, and Angular authority/task resolution.
...
1. shipped package behavior and published schemas for 3.11.3;
...
Describe 3.11.3 as released only after package closeout passes. Do not describe it as value-proven until the separate frozen A/B research gates pass.
