...
| `@decantr/cli` | core-supported | `public-delivery` | stable | `delivery` (`30`) | `stable` | `latest` | `true` | `true` | `0` | `stable-public` | Workflow commands for content-first setup, authority-aware scan/adopt/task, doctor, verify, CI, codify, connect, and content operations, including Adoption Truth, bounded Task Capsules, Governance Deltas, default CI v2, opt-in CI v3, monorepo targeting, health baselines, and read-only report-isolated Studio. |
