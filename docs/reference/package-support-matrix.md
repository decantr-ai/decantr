...
| `@decantr/verifier` | core-supported | `public-operator` | stable | `delivery` (`10`) | `latest` | `true` | `true` | `0` | `stable-public` | Shared verification, critique, contract assertions, Evidence Bundle, Project Health, Workspace Health, and report-schema engine. |
| `@decantr/mcp-server` | core-supported | `public-operator` | stable | `delivery` (`20`) | `latest` | `true` | `true` | `0` | `stable-public` | MCP delivery surface for Decantr design intelligence, packs, evidence bundles, workspace health, and repair prompts. |
| `@decantr/cli` | core-supported | `public-delivery` | stable | `delivery` (`30`) | `latest` | `true` | `true` | `0` | `stable-public` | Local workflow/adoption policy resolution, adapter-backed bootstrap/attach, registry sync, Evidence Bundles, workspace health, Project Health CI and Studio, Content Health, and project governance. |
