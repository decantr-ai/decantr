...
| `@decantr/registry` | library | read: Explicit local content JSON paths passed to resolver helpers<br>write: none<br>delete: none | outbound: Configured Decantr API base URL for 3.x compatibility content search, resolve, and hosted pack helpers<br>inbound: no | none | none | caller-controlled | none | dist, schema, README.md, package.json |
