...
- Auth exit assurance is stricter now too: if reviewed source files persist auth state in browser storage or client-managed cookies, both project audit and file critique now expect those persistence surfaces to be explicitly cleared during sign-out instead of assuming redirects or generic sign-out helpers are sufficient.
