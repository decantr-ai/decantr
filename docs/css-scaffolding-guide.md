# Legacy Decantr CSS Adapter Guide

This compatibility guide applies only to projects that explicitly select the `decantr-css` adoption mode. `@decantr/css` is a legacy optional adapter, not a Decantr default and not a requirement for contract-only or style-bridge adoption. New and Brownfield projects should keep their existing styling authority unless they deliberately opt into this adapter.

---

## Quick Start

The optional Decantr CSS adapter has two parts:
...
Use this setup only when the project contract and `package.json` already select `decantr-css`, or when a maintainer explicitly chooses that adoption mode.
...
- In an explicit `decantr-css` project, use the real `@decantr/css` runtime for atoms. Do not add it to contract-only or style-bridge projects.
...
The following sections cover manual implementation inside the explicit `decantr-css` compatibility lane.

---

## Adapter Layer Structure

Projects using the legacy adapter must declare this CSS `@layer` order so adapter treatments, decorators, and app styles remain deterministic. Other adoption modes follow the host project's existing cascade strategy.
...
The canonical layer order for projects using the legacy adapter:
...
When implementing the explicit `decantr-css` adapter:
...
Shell `internal_layout` provides semantic per-region specs. Contract-only and style-bridge projects translate those specs into their existing styling authority; only explicit adapter projects consume legacy `@decantr/css` atom hints:
...
Shell data uses **semantic properties** (`width`, `padding`, `gap`, `scroll`) as the source of truth. Legacy atom hints such as `_flex`, `_gap4`, and `_p6` apply only to the explicit adapter. This keeps shells framework-agnostic while the implementation translates semantic specs into the host project's styling approach.
...
- [Decantr CSS package compatibility status](../packages/css/README.md)
