import { css, getStyle, getMode } from 'decantr/css';
...
function colorSwatch(token) {
  const valueEl = span({ class: 'de-swatch-value' });
  createEffect(() => {
    getStyle(); getMode();
    const val = getComputedStyle(document.documentElement).getPropertyValue(token).trim();
    valueEl.textContent = val || 'unset';
  });

  return div({ class: 'de-swatch' },
    div({ class: 'de-swatch-color', style: `background:var(${token},transparent)` }),
...
function chromeSection() {
  const tokens = ['--d-chrome-bg', '--d-chrome-fg', '--d-chrome-border', '--d-chrome-muted', '--d-chrome-hover', '--d-chrome-active'];
  return div({ class: 'de-section-block' },
    h3({ class: 'de-section-title' }, 'Chrome'),
    p({ class: css('_caption _fgmutedfg _mb3') }, 'Inverted navigation chrome for header/sidebar'),
    div({ class: 'de-token-grid' }, ...tokens.map(t => colorSwatch(t)))
  );
}
    ...
    div({ class: css('_grid _gc4 _gap3') },
      ...levels.map(i => {
        ...
        createEffect(() => {
          ...
          bgLabel.textContent = `bg: ${style.getPropertyValue(`--d-surface-${i}`).trim()}`;
          fgLabel.textContent = `fg: ${style.getPropertyValue(`--d-surface-${i}-fg`).trim()}`;
        });

        return div({
          class: 'de-surface-card',
          style: `background:var(--d-surface-${i});color:var(--d-surface-${i}-fg);border:1px solid var(--d-surface-${i}-border)`
        }, strong({}, `Surface ${i}`), bgLabel, fgLabel);
      })
    )
  );
}
    ...
    div({ class: css('_flex _col _gap1') },
      ...sizes.map(([token]) => {
        ...
        createEffect(() => {
          getStyle(); getMode();
          const val = getComputedStyle(document.documentElement).getPropertyValue(token).trim();
          sampleEl.textContent = `The quick brown fox (${val})`;
        });

        return div({ class: 'de-type-sample' },
          span({ class: 'de-type-label' }, token),
          span({ style: `font-size:var(${token},1rem)` }, sampleEl)
        );
      })
    )
  );
  ...
  return div({ class: 'de-section-block' },
    ...
    div({ class: css('_flex _col _gap1') },
      ...spacings.map(([token, fallback]) => {
        const valLabel = span({ class: 'de-spacing-label' });
        createEffect(() => {
          getStyle(); getMode();
          const val = getComputedStyle(document.documentElement).getPropertyValue(token).trim() || fallback;
          valLabel.textContent = val;
        });

        return div({ class: 'de-spacing-row' },
          span({ class: 'de-spacing-label' }, token.replace('--d-', '')),
          div({ class: 'de-spacing-bar', style: `width:var(${token},${fallback})` }),
          valLabel
        );
      })
    )
...
function elevationSection(data) {
  ...
  return div({ class: 'de-section-block' },
    ...
    div({ class: css('_grid _gc4 _gap4') },
      ...levels.map(i =>
        div({ class: 'de-elevation-box', style: `box-shadow:var(--d-elevation-${i})` }, `elevation-${i}`)
      )
    )
  );
}
    ...
    div({ class: css('_flex _gap3 _wrap') },
      ...durations.map(token => {
        const box = div({
          class: 'de-motion-box',
          title: token,
          style: `transition:transform var(${token},0s) var(--d-easing-standard)`
        });

        createEffect(() => {
          getStyle(); getMode();
          const val = getComputedStyle(document.documentElement).getPropertyValue(token).trim() || '0s';
          box.title = `${token}: ${val}`;
        });
    ...
    div({ class: css('_flex _gap2 _wrap') },
      ...tokens.map(token =>
        div({ class: css('_flex _col _aic _gap1') },
          div({ class: css('_w[3rem] _h[3rem] _radius _b1 _bcborder'), style: `background:var(${token})` }),
    ...
    if (renderFn && data[group]) {
      ...
      for (const [id, groupData] of Object.entries(data)) {
        ...
        if (fn) {
          ...
          el.id = 'tokens-' + id;
          sections.push(el);
          // Insert chrome section after neutral
          if (id === 'neutral') {
            const chrome = chromeSection();
            chrome.id = 'tokens-chrome';
