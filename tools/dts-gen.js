...
function genBlocks() {
  const source = readSource('src/blocks/index.js');
  const names = getExportedNames(source);
...
`;
   ...
   * ${name} content block.
   */
  export function ${name}(props?: Record<string, any>): HTMLElement;
...
function genKitDashboard() {
  const source = readSource('src/kit/dashboard/index.js');
  const names = getExportedNames(source);
...
`;
  ...
  for (const name of names) {
    decls += `  /**
   * Dashboard kit: ${name} component.
   * Props accept signal getters or static values.
   */
  export function ${name}(props?: Record<string, any>, ...children: any[]): HTMLElement;
...
function genKitAuth() {
  const source = readSource('src/kit/auth/index.js');
  const names = getExportedNames(source);
...
function genKitContent() {
  const source = readSource('src/kit/content/index.js');
  const names = getExportedNames(source);
...
function main() {
  ...
  const generators = [
    ...
    ['icons.d.ts', genIcons],
    ['chart.d.ts', genChart],
    ['blocks.d.ts', genBlocks],
    ['kit-dashboard.d.ts', genKitDashboard],
    ['kit-auth.d.ts', genKitAuth],
    ['kit-content.d.ts', genKitContent],
    ['test.d.ts', genTest],
    ['index.d.ts', genIndex],
  ];
  ...
  const mapping = {
    ...
    'decantr/blocks': 'types/blocks.d.ts',
    'decantr/kit/dashboard': 'types/kit-dashboard.d.ts',
    'decantr/kit/auth': 'types/kit-auth.d.ts',
    'decantr/kit/content': 'types/kit-content.d.ts',
