...
function AnnouncementBar() {
  ...
  return cond(visible,
    () => div({ class: css('_flex _aic _jcc _gap3 _py2 _px4 _textsm d-glass-subtle') },
      ...
      span({}, 'Decantr v0.4.2 is here — new patterns, presets, and more'),
...
function BrandHero() {
  return section({ class: css('_flex _col _aic _tc _gap8 _py24 _px6 _minh[100vh] _jcc _relative d-mesh') },
    ...
    // Content
    Badge({ variant: 'outline', icon: 'zap', class: css('aura-shimmer') }, 'AI-First Framework'),
    h1({ class: css('d-heading-display d-gradient-text _mw[900px]') }, 'Build stunning apps in minutes, not months'),
...
function CodePreviewSection() {
  // Use array join to prevent dev server import-map rewriting of bare specifiers
  const pkg = ['decan', 'tr'].join('');
  const code = [
    `import { tags } from '${pkg}/tags';`,
    `import { createSignal } from '${pkg}/state';`,
    `import { Button, Card } from '${pkg}/components';`,
    `import { css } from '${pkg}/css';`,
    ``,
    `function Counter() {`,
    `  const { div, h2 } = tags;`,
  ...
  return section({ class: css('_grid _gc1 _lg:gc2 _gap8 _py24 _px6 _aic _mw[1100px] _mx[auto]') },
    div({ class: css('_flex _col _gap6') },
      ...
      p({ class: css('_body _fgmuted _lh[1.6]') },
        'Write declarative UI with real DOM elements, reactive signals, and atomic styling — all in plain JavaScript. No JSX, no compilation step.'
      ),
      div({ class: css('_flex _gap2 _wrap') },
        ...
        Chip({ icon: 'file-text', label: 'TypeScript Types' })
      )
    ),
...
function TestimonialWall() {
  const testimonials = [
    ...
    { quote: 'The AI-first approach means I can scaffold an entire dashboard in minutes. Game changer for prototyping.', name: 'Marcus Johnson', role: 'Senior Engineer at Vercel' },
    { quote: 'Zero dependencies. That alone sold us. But the component quality rivals Radix and Shadcn.', name: 'Priya Patel', role: 'Tech Lead at Stripe' },
    { quote: 'We migrated from Next.js in a weekend. The atomic CSS system is what Tailwind should have been.', name: 'James Wright', role: 'Founder at DevTools Co' },
    { quote: 'The Decantation Process is brilliant. It forces you to think about structure before code. Our team ships cleaner work now.', name: 'Elena Volkov', role: 'Engineering Manager at Linear' },
    { quote: 'Best docs I have seen for any framework. The registry system makes code generation actually reliable.', name: 'Tomás Rivera', role: 'Staff Engineer at Notion' },
    { quote: 'Auradecantism is gorgeous out of the box. Our designers were impressed without any custom CSS.', name: 'Aisha Okafor', role: 'Product Designer at Figma' },
    { quote: 'The enterprise features — form validation, error boundaries, route guards — saved us months of work.', name: 'David Kim', role: 'VP Engineering at Acme' },
    ...
    div({ class: css('_grid _gc1 _md:gc4 _gap8 _py12 _px6 _mw[1100px] _mx[auto]') },
      div({ class: css('_flex _col _gap3') },
        ...
        div({ class: css('_flex _gap3 _mt2') },
          a({ href: '#', 'aria-label': 'GitHub', class: css('_fgmuted _h:fgfg') }, icon('github')),
          a({ href: '#', 'aria-label': 'Twitter', class: css('_fgmuted _h:fgfg') }, icon('twitter')),
