...
function expressionLooksLikeOpenRedirectQueryGetterFunction(
  ...
  if (
    isCallLikeExpression(expression)
    && isMemberAccessExpression(expression.expression)
    && isMemberAccessNamed(expression.expression, 'trim', 'trimStart', 'trimLeft', 'trimEnd', 'trimRight', 'replace', 'replaceAll', 'substr', 'substring', 'toLowerCase', 'toUpperCase', 'toLocaleLowerCase', 'toLocaleUpperCase', 'normalize')
    && expressionLooksLikeOpenRedirectQueryGetterFunction(
      expression.expression.expression,
      sourceFile,
      namedExpressions,
