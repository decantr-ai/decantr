...
describe('v3-aware tool tests', () => {
  ...
  describe('decantr_check_drift — v3 separated response', () => {
    ...
    it('should check components_used against page layout', async () => {
      ...
      expect(result.drifted).toBe(true);
      const compViolation = result.blueprint_drift.find(
        (v) => v.rule === 'component-pattern-match',
      );
  ...
  describe('decantr_get_execution_pack', () => {
    it('returns scaffold task, scaffold overview, and compiled pack together', async () => {
      ...
      await writeFile(
        join(contextDir, 'scaffold-pack.json'),
        JSON.stringify({
          packType: 'scaffold',
    ...
    it('falls back to hosted scaffold packs when local context artifacts are missing', async () => {
              ...
              sections: [
                {
                  ...
                  pages: [{ id: 'overview', route: '/', layout: ['kpi-grid'] }],
                },
              ],
              features: ['auth'],
            },
      ...
      expect(
        fetchSpy.mock.calls.filter(([input]) => String(input).includes('/v1/packs/select')),
      ).toHaveLength(2);
    });
      ...
      const fetchSpy = vi.spyOn(globalThis, 'fetch').mockImplementation((input) => {
        ...
        if (url.includes('/v1/packs/select')) {
          return Promise.resolve(
            new Response(JSON.stringify(makeHostedSelectedPack('scaffold')), {
              status: 200,
              headers: { 'Content-Type': 'application/json' },
            }),
          );
    ...
    it('returns a page pack together with parent section context when available', async () => {
      ...
      await writeFile(
        join(contextDir, 'page-overview-pack.md'),
        '# Page Pack\n\n- Page: overview\n',
      );
      await writeFile(
        join(contextDir, 'page-overview-pack.json'),
        JSON.stringify({
          packType: 'page',
      ...
      await writeFile(
        join(contextDir, 'section-dashboard-pack.md'),
        '# Section Pack\n\n- Section: dashboard\n',
      );
      await writeFile(
        join(contextDir, 'section-dashboard-pack.json'),
        JSON.stringify({
          packType: 'section',
    ...
    it('falls back to hosted page packs when the local manifest is missing', async () => {
      ...
      const fetchSpy = vi.spyOn(globalThis, 'fetch').mockImplementation((input, init) => {
        ...
        if (url.includes('/v1/packs/select')) {
          ...
          const parsed =
            typeof body === 'string'
              ? (JSON.parse(body) as { pack_type?: string; id?: string })
              : {};
          if (parsed.pack_type === 'page' && parsed.id === 'overview') {
            return Promise.resolve(
              new Response(JSON.stringify(makeHostedSelectedPack('page', 'overview')), {
                status: 200,
                headers: { 'Content-Type': 'application/json' },
              }),
            );
      ...
      expect(
        fetchSpy.mock.calls.filter(
          ([input]) => typeof input === 'string' && input.includes('/v1/packs/select'),
        ),
      ).toHaveLength(2);
    });
    ...
    it('returns a specific mutation pack in markdown and json', async () => {
      ...
      await writeFile(
        join(contextDir, 'mutation-modify-pack.md'),
        '# Mutation Pack\n\n- Operation: modify\n',
      );
      await writeFile(
        join(contextDir, 'mutation-modify-pack.json'),
        JSON.stringify({
          packType: 'mutation',
