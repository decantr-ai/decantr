...
export function parse(tokens, file) {
  ...
  function advance() {
    ...
    if (token && token.loc && token.loc.offset !== undefined) {
      lastConsumedEnd = token.loc.offset + token.value.length;
    }
    return token;
  }
  ...
  function parseImport() {
    ...
    const startOffset = current().loc.offset;
    ...
    if (matchPunc('(')) {
      ...
      expect('punctuator', ')');
      ast.imports.push({ type: 'dynamic', source, loc: startLoc, start: startOffset, end: lastConsumedEnd });
      return;
    }
    ...
    if (match('string')) {
      ...
      ast.imports.push({ type: 'side-effect', source, loc: startLoc, start: startOffset, end: lastConsumedEnd });
      return;
    }
    ...
    if (match('identifier')) {
      ...
      if (matchPunc(',')) {
        ...
        // Consume trailing semicolon if present
        if (matchPunc(';')) advance();
        ast.imports.push({ type: 'default', source, name: defaultName, loc: startLoc, start: startOffset, end: lastConsumedEnd });
        return;
      }
    }
    ...
    // Consume trailing semicolon if present
    if (matchPunc(';')) advance();
    const endOffset = lastConsumedEnd;

    if (defaultName) {
      ast.imports.push({ type: 'default', source, name: defaultName, loc: startLoc, start: startOffset, end: endOffset });
    }
    if (names.length > 0) {
      ast.imports.push({ type: 'named', source, names, loc: startLoc, start: startOffset, end: endOffset });
    }
    if (namespace) {
      ast.imports.push({ type: 'namespace', source, name: namespace, loc: startLoc, start: startOffset, end: endOffset });
    }
  }
  ...
  function parseExport() {
    ...
    if (matchKeyword('default')) {
      advance();
      const declStart = current().loc ? current().loc.offset : lastConsumedEnd;
      ...
      if (matchKeyword('function') || matchKeyword('class') || matchKeyword('async')) {
        ...
        ast.exports.push({ type: 'default', name, loc: startLoc, start: startOffset, end: lastConsumedEnd, declStart });
      } else {
        // Expression - skip to semicolon or end
        skipExpression();
        ast.exports.push({ type: 'default', name: null, loc: startLoc, start: startOffset, end: lastConsumedEnd, declStart });
      }
      return;
    }
    ...
    const isAsync = matchKeyword('async');
    const asyncDeclStart = isAsync ? current().loc.offset : undefined;
    if (isAsync) advance();

    if (matchKeyword('const') || matchKeyword('let') || matchKeyword('var')) {
      const declStart = current().loc.offset;
      ...
      // Update end after skipping statement
      for (const exp of ast.exports) {
        if (exp.start === startOffset) {
          exp.end = lastConsumedEnd;
        }
      }
      return;
    }
    ...
    if (matchKeyword('function')) {
      const declStart = isAsync ? asyncDeclStart : current().loc.offset;
      advance();
