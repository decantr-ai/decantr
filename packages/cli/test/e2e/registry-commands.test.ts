...
describe('registry commands (e2e)', () => {
  ...
  it('registry compile-packs posts an essence document and returns a pack bundle', async () => {
    ...
    expect(
      requests.some((request) => {
        if (
          request.method !== 'POST' ||
          !request.url?.includes('/v1/packs/compile?namespace=%40official') ||
          !request.body
        ) {
          return false;
        }
  ...
  it('registry get-pack posts an essence document and returns a selected pack', async () => {
    ...
    expect(
      requests.some((request) => {
        if (
          request.method !== 'POST' ||
          !request.url?.includes('/v1/packs/select?namespace=%40official') ||
          !request.body
        ) {
          return false;
        }
  ...
  it('registry get-pack can write a selected hosted pack into .decantr/context', async () => {
    ...
    await runCliAsync(
      testDir,
      'registry get-pack page home --namespace @official --write-context',
      {
        DECANTR_API_URL: `http://127.0.0.1:${port}/v1`,
        DECANTR_API_KEY: '',
      },
  ...
  it('registry critique-file posts local source and essence to the hosted verifier', async () => {
    ...
    const output = await runCliAsync(
      testDir,
      'registry critique-file Home.tsx --namespace @official --json',
      {
        DECANTR_API_URL: `http://127.0.0.1:${port}/v1`,
        DECANTR_API_KEY: '',
      },
  ...
  it('registry audit-project posts essence, optional dist, and optional source snapshots to the hosted verifier', async () => {
    ...
    writeFileSync(
      ...
      '<!doctype html><html lang="en"><head><meta name="viewport" content="width=device-width, initial-scale=1"><title>Audit</title></head><body><div id="root"></div><script type="module" src="/assets/app.js"></script></body></html>\n',
      { encoding: 'utf-8' },
    );
    ...
    const output = await runCliAsync(
      testDir,
      'registry audit-project --namespace @official --json --sources src',
      {
        DECANTR_API_URL: `http://127.0.0.1:${port}/v1`,
        DECANTR_API_KEY: '',
      },
    ...
    expect(
      requests.some(
        (url) =>
          ...
          url.includes('recommended=true') &&
          url.includes('intelligence_source=hybrid'),
      ),
    ).toBe(true);
    expect(
      requests.some(
