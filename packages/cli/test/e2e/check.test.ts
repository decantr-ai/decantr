...
describe('check command (e2e)', () => {
  ...
  it('uses local registry cache for pattern guard validation', () => {
    ...
    writeFileSync(join(testDir, '.decantr', 'cache', '@official', 'patterns', 'hero.json'), JSON.stringify({
      id: 'hero',
      name: 'Hero',
      version: '1.0.0',
    }, null, 2));
    writeFileSync(join(testDir, '.decantr', 'cache', '@official', 'themes', 'luminarum.json'), JSON.stringify({
      id: 'luminarum',
      modes: ['dark', 'light'],
      version: '1.0.0',
    }, null, 2));
