...
describe('analyze command', () => {
  ...
  it('recognizes React Router and Decantr starter structure in an attached app', () => {
    ...
    writeFileSync(join(testDir, 'src', 'main.tsx'), 'import { HashRouter } from "react-router-dom";\n');
    writeFileSync(join(testDir, 'src', 'App.tsx'), 'import { Routes, Route } from "react-router-dom";\nexport function App() { return <Routes><Route path="/" element={<main>Hello</main>} /></Routes>; }\n');
    writeFileSync(join(testDir, 'src', 'styles', 'global.css'), 'html.dark { color-scheme: dark; }\n');
    writeFileSync(join(testDir, 'src', 'styles', 'tokens.css'), ':root { --d-primary: #111827; --d-border: #d1d5db; }\n');
    writeFileSync(join(testDir, 'src', 'styles', 'treatments.css'), '.d-surface { background: var(--d-primary); }\n');
    ...
    const analysis = JSON.parse(readFileSync(join(testDir, '.decantr', 'analysis.json'), 'utf-8')) as {
