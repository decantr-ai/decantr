...
Decantr 3.11.2 is the current stable product line; this foundation package remains at 3.10.0. Core graph and pack primitives support Changed-UI Assurance and the deeper **Observe -> Prepare -> Verify -> Report** loop, but a graph, route rank, or larger context pack does not by itself prove production authority or model improvement. Routes are one UI surface among layouts, components, stories, overlays, flows, packages, runtime states, and exact source files.
