...
import type { ApiContentType, Archetype, Blueprint, Pattern, Shell, Theme } from './types.js';

const DEFAULT_BASE_URL = 'https://api.decantr.ai/v1';
const DEFAULT_TIMEOUT_MS = 30000;
const DEFAULT_CACHE_TTL_MS = 5 * 60 * 1000; // 5 minutes

function readEnvironmentVariable(name: 'DECANTR_API_URL' | 'REGISTRY_URL'): string | undefined {
...
export class ContentAPIClient {
  ...
  constructor(options: ContentAPIClientOptions = {}) {
    this.baseUrl =
      options.baseUrl ??
      readEnvironmentVariable('DECANTR_API_URL') ??
      readEnvironmentVariable('REGISTRY_URL') ??
      DEFAULT_BASE_URL;
  ...
  protected createError(status: number, message: string, details?: unknown): ContentAPIError {
    return new ContentAPIError(status, message, details);
  }
  ...
  private async request<T>(path: string, init?: RequestInit): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw this.createError(response.status, `API ${response.status}: ${message}`, details);
    }
  ...
  /** @deprecated Use getContentIntelligenceSummary(). */
  async getRegistryIntelligenceSummary(params?: {
    namespace?: string;
  }): Promise<ContentIntelligenceSummaryResponse> {
    return this.getContentIntelligenceSummary(params);
  }

  async getContentIntelligenceSummary(params?: {
    namespace?: string;
  }): Promise<ContentIntelligenceSummaryResponse> {
    const cacheKey = `registry-intelligence-summary:${JSON.stringify(params ?? {})}`;
    const cached = this.getCached<ContentIntelligenceSummaryResponse>(cacheKey);
    if (cached) return cached;

    const searchParams = new URLSearchParams();
    if (params?.namespace) searchParams.set('namespace', params.namespace);
    const query = searchParams.toString();
    const result = await this.request<ContentIntelligenceSummaryResponse>(
...
export function createContentAPIClient(options: ContentAPIClientOptions = {}): ContentAPIClient {
  return new ContentAPIClient(options);
}

// Lightweight content client retained for the legacy createRegistryClient facade.
...
export interface ContentClient {
  search(query: string, type?: string): Promise<ContentSearchResult[]>;
...
export function createContentClient(options: ContentClientOptions = {}): ContentClient {
  ...
  return {
    async search(query: string, type?: string): Promise<ContentSearchResult[]> {
      const params = new URLSearchParams({ q: query });
      if (type) params.set('type', type);
      const res = await fetch(`${baseUrl}/search?${params}`);
      if (!res.ok) return [];
      const data = (await res.json()) as
        | { results?: ContentSearchResult[]; total?: number }
        | ContentSearchResult[];
      // API returns { total, results } wrapper
      if (Array.isArray(data)) return data;
      return (data as { results: ContentSearchResult[] }).results ?? [];
    },
