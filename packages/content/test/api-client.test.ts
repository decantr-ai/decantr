...
import { ContentAPIClient, ContentAPIError } from '../src/api-client.js';
...
describe('ContentAPIClient showcase endpoints', () => {
  it('surfaces failed responses as canonical ContentAPIError instances', async () => {
    ...
    const client = new ContentAPIClient({ baseUrl: 'https://api.example.com/v1' });
    ...
    const result = await client.auditProject(
      {
        ...
        dist: {
          indexHtml:
            '<!doctype html><html><head><title>Audit</title></head><body><div id="root"></div><script type="module" src="/assets/app.js"></script></body></html>',
          assets: {
